...
public final class FoliaRaceFixturePlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        switch (scenario) {
            ...
            case "async-state-access" -> Bukkit.getAsyncScheduler().runNow(this, task -> observe(distant, "async"));
    ...
    private void observe(Location target, String label) {
        ...
        getLogger().info("fixture scenario=" + label + ", accepted=" + receipt.accepted()
                + ", ownershipAvailable=" + receipt.ownershipCheckAvailable()
                + ", ownsTarget=" + receipt.ownedByCurrentContext());
    }
}
